...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    description:
      ...
      "and tool results, conclusions = last 3 assistant texts, conversation = user+assistant text only); 'search' scans session text across projects. Read-only.",
    promptSnippet:
    ...
    parameters: Type.Object({
      ...
      offset: Type.Optional(
        Type.Integer({
          minimum: 0,
          description: "messages view: skip N messages",
        }),
      ),
      limit: Type.Optional(
        Type.Integer({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (params.action === "sessions") {
        ...
        if (!target) {
          throw new Error(
            `no project '${params.project}' (known: ${
        ...
        if (sessions.length === 0) {
          return {
            content: [{
              type: "text",
              text: `no sessions in '${target.slug}'`,
            }],
      ...
      switch (view) {
        ...
        case "conclusions":
          return {
            content: [
              {
                type: "text",
                text: renderConclusions(resolved.session, parsed),
              },
            ],
            details: { address: params.address, id: resolved.session.id },
          };
        case "conversation":
          return {
            content: [
              {
                type: "text",
                text: renderConversation(resolved.session, parsed),
              },
            ],
            details: { address: params.address, id: resolved.session.id },
          };
        ...
        default:
          throw new Error(
            `unknown view '${
              String(view)
            }' (expected summary, tree, messages, or message)`,
          );
      }
    },
  });
