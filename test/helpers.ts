...
export function fakeRepo(
	...
	switch (kind) {
		case "git":
			...
			if (markerId)
				writeFileSync(join(root, ".git", "blueberry-id"), markerId + "\n");
			break;
		case "lore":
			mkdirSync(join(root, ".lore"), { recursive: true });
			if (markerId)
				writeFileSync(join(root, ".lore", "blueberry-id"), markerId + "\n");
			break;
		case "worktree":
			mkdirSync(root, { recursive: true });
			writeFileSync(
				join(root, ".git"),
				"gitdir: /elsewhere/main/.git/worktrees/wt\n",
			);
			if (markerId) {
...
export function fakeSession(storeDir: string, spec: SessionSpec): string {
	...
	if (spec.firstUserText !== undefined)
		pushEntry("message", {
			message: {
				role: "user",
				content: spec.firstUserText,
				timestamp: Date.now(),
			},
		});
	if (spec.name !== undefined) pushEntry("session_info", { name: spec.name });
	for (let i = 0; i < (spec.entries ?? 0); i++)
		pushEntry("message", {
			message: {
				role: "assistant",
				content: [{ type: "text", text: `dummy ${i}` }],
				usage: null,
				stopReason: "stop",
				timestamp: Date.now(),
			},
