...
import { findBySlug, loadRegistry, mutations } from "../src/core/registry.ts";
...
test("resolveAddress: purely numeric uuid prefix falls through index miss", () => {
  ...
  assert.throws(
    () => resolveAddress(registry, agentDir, "numeric", "numeric/9999"),
    /out of range/,
  );
});
...
test("forkSession: copies into target store, names name@project, source intact", () => {
  ...
  // source untouched
  assert.equal(
    listSessions(getCentralStoreDir(agentDir, "src-proj")).length,
    1,
  );
...
test("forkSession: unnamed session derives name from first text; slashes stripped", () => {
  ...
  assert.ok(
    result.name.startsWith("fix the parser bug - quickly".slice(0, 20)),
  );
...
test("branch sweep: degenerate entries, empty results, truncation, fork fallbacks", () => {
  ...
  assert.ok(
    renderTree(parseSessionFile(bigFile)!).includes("[tree truncated]"),
  );
...
// --- conclusions + conversation views (feedback §1 gaps 2-3) ------------------

test("renderConclusions: last 3 assistant texts, tools skipped, empty session handled", () => {
  const store = getCentralStoreDir(agentDir, "views");
  const file = richSession(store, "rich-session");
  const parsed = parseSessionFile(file)!;
  const text = renderConclusions(
    listSessions(store).find((s) => s.file === file)!,
    parsed,
  );
  // rich-session's assistant texts end with "Running them now." — present
  assert.ok(text.includes("Running them now."));
  assert.ok(text.match(/--- #\d+ ---/), "seq markers render");
  assert.ok(!text.includes("[toolResult]"), "tool noise excluded");
});

test("renderConversation: user+assistant text only, tool calls dropped", () => {
  const store = getCentralStoreDir(agentDir, "views");
  const file = richSession(store, "rich-session");
  ...
  assert.ok(text.includes("run the tests please"));
  assert.ok(!text.includes("· tool"), "tool call one-liners dropped");
  assert.ok(!text.includes("toolResult"), "tool results dropped");
});

test("renderConversation: trims to last 200 with marker", () => {
  const store = getCentralStoreDir(agentDir, "views");
  const file = fakeSession(store, {
    cwd: store,
    firstUserText: "start",
  ...
  assert.ok(text.startsWith("… (older messages trimmed)"), "trim marker");
  // 250 filler entries → far over 200 lines; last content survives
