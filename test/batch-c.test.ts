...
test("main: cmd-executor without id errors; with unknown graph errors", async () => {
	...
	assert.equal(
		await main(["cmd-executor", "00000000-0000-0000-0000-000000000000"], d2),
		1,
	);
...
test("main: cmd-executor runs a defined graph to done", async () => {
	...
	const nodes = db
		.prepare("SELECT status FROM cmd_nodes WHERE graph_id = ?")
...
test("main: import with unknown --from source scans nothing, exits 0", async () => {
	...
	assert.ok(
		out.some((l) => l.includes("would import") || l.includes("imported")),
	);
});
...
test("import-history: apply with a real project lands aiTitle as session name", () => {
	...
	writeFileSync(
		...
		[
			'{"type":"user","uuid":"u1","parentUuid":null,"sessionId":"t1","timestamp":"2026-01-01T00:00:00Z","cwd":"' +
				area +
				'","message":{"role":"user","content":"q"}}',
			'{"type":"assistant","uuid":"u2","parentUuid":"u1","sessionId":"t1","timestamp":"2026-01-01T00:00:01Z","cwd":"' +
				area +
				'","message":{"role":"assistant","content":"a"},"aiTitle":"My Titled Chat"}',
		].join("\n") + "\n",
	);
	db
		.prepare(
	...
	const name = db.prepare("SELECT name FROM sessions WHERE id = 't1'").get() as {
		name: string | null;
	};
...
test("updater: sidecar-missing asset refuses install", async () => {
	...
	await assert.rejects(
		() => performUpdate("0.1.0", "darwin-aarch64", io),
		/checksum mismatch/,
	);
});
...
test("plan-store: seedPlan creates todos from steps (error arm is defensive: titles are S{n}.-prefixed, never empty)", () => {
	...
	const todos = db
		.prepare(
			"SELECT title FROM todos WHERE project_id = 'pp' ORDER BY created_at",
		)
