...
function deps(cwd: string): CliDeps {
	return { cwd, agentDir, spawn: async () => 0, out: (l) => out.push(l), err: () => {}, gitRemoteReader: () => null };
}
...
test("pane: ageMs orders seconds/minutes and tolerates junk", () => {
	const mk = (age: string) => ({ id: age, title: age, stage: "review" as const, age });
...
test("store: minute-scale age renders as Xm", () => {
	...
	const pid = "pts";
	db.prepare("INSERT INTO projects (id, slug, canonical_path, created_at, updated_at) VALUES (?, 'p', '/p', ?, ?)").run(
		pid,
...
test("store: done→todo reopen passes legality block (stage not in gate set)", () => {
	const db = openDb(agentDir);
	const pid = "pts2";
	db.prepare("INSERT INTO projects (id, slug, canonical_path, created_at, updated_at) VALUES (?, 'p2', '/p2', ?, ?)").run(
		pid,
		new Date().toISOString(),
		new Date().toISOString(),
	);
...
test("sync: empty-string cwd header is treated as no cwd; missing timestamp tolerated", () => {
	...
	writeFileSync(noCwd, `${JSON.stringify({ type: "session", version: 3, id: "nocwd-00000001", timestamp: new Date().toISOString(), cwd: "" })}\n`);
	const noTs = join(store, "nots.jsonl");
	writeFileSync(noTs, `${JSON.stringify({ type: "session", version: 3, id: "nots-00000001", cwd: root })}\n`);

	const db = openDb(agentDir);
	assert.equal(ingestSessionFile(db, noCwd, (c) => (c === root ? projectId : null)).status, "orphan", "empty cwd → orphan");
	const okRes = ingestSessionFile(db, noTs, (c) => (c === root ? projectId : null));
...
test("search: session rows without a project render bare headers", () => {
	...
	db.prepare("INSERT INTO session_fts (text, session_id, seq, role) VALUES ('floating needle text', 'nop-1', 0, 'user')").run();
	db.prepare(
		"INSERT INTO session_entries (session_id, seq, ts, type, entry_id, parent_id, json) VALUES ('nop-1', 0, NULL, 'message', NULL, NULL, ?)",
	).run(JSON.stringify({ type: "message", id: "e", message: { role: "user", content: "floating needle text" } }));
...
test("library: numeric message content renders empty without crashing", () => {
	...
	writeFileSync(
		file,
		[
			JSON.stringify({ type: "session", version: 3, id: "num-0000000001", timestamp: ts, cwd: "/x" }),
...
test("sessions: invalid first line skips the whole file", () => {
	...
	mkdirSync(store, { recursive: true });
	writeFileSync(join(store, "bad.jsonl"), "garbage first line\n{\"type\":\"message\"}\n");
...
test("cli: search --context with junk is ignored; --project at argv end", async () => {
	...
	assert.equal(await main(["search", "anything", "--context", "abc"], deps(root)), 0); // junk context ignored
