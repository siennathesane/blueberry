...
test("client: capabilities include valueSet (deno requirement)", () => {
  ...
  assert.ok(
    Array.isArray(kind["valueSet"]) &&
      (kind["valueSet"] as number[]).length > 0,
...
test("friendlyError: server quirks mapped to readable messages", () => {
  ...
  assert.ok(
    LspManager.friendlyError(
      "rename",
      new Error(
        'renaming "Greet" to "X" not possible because "pkg" has errors',
      ),
...
import { chmodSync as ch, mkdirSync as mk, rmSync as rmf } from "node:fs";
...
// --- F1: toOneBasedLines unit tests ---

import { emptyText, toOneBasedLines } from "../extensions/lsp/index.ts";

test("toOneBasedLines: shifts foldingRange startLine and endLine by +1", () => {
  const input = [
    { startLine: 0, endLine: 5, kind: "region" },
    { startLine: 10, endLine: 15, kind: "comment" },
  ];
  const result = toOneBasedLines("foldingRange", input);
  assert.deepEqual(result, [
    { startLine: 1, endLine: 6, kind: "region" },
    { startLine: 11, endLine: 16, kind: "comment" },
  ]);
});

test("toOneBasedLines: shifts documentHighlight range lines by +1", () => {
  const input = [
    {
      range: {
  ...
  const result = toOneBasedLines("documentHighlight", input);
  assert.deepEqual(result, [
    {
      range: {
...
test("toOneBasedLines: passthrough for semanticTokens and other actions", () => {
  const data = { data: [1, 2, 3] };
  assert.equal(toOneBasedLines("semanticTokens", data), data);
  assert.deepEqual(toOneBasedLines("codeLens", []), []);
  assert.equal(toOneBasedLines("selectionRange", null), null);
  assert.equal(toOneBasedLines("documentLink", undefined), undefined);
});

// --- F2: emptyText unit tests ---

test("emptyText: returns no symbol at position when hasPosition is true", () => {
  assert.equal(emptyText(true), "no symbol at position");
});

test("emptyText: returns no results when hasPosition is false", () => {
  assert.equal(emptyText(false), "no results");
});

// --- F3: workspaceSymbol guard (source-grep assertion) ---
// The workspaceSymbol guard lives inside the tool execute closure, which is not
// directly exportable. We verify the exact error string exists in the source.
test("workspaceSymbol guard: error message is present in extension source", async () => {
  const { readFile } = await import("node:fs/promises");
  const src = await readFile(
    new URL("../extensions/lsp/index.ts", import.meta.url).pathname,
    "utf8",
  );
  assert.ok(
    src.includes("workspaceSymbol needs a file path to infer the language"),
    "guard error string found in source",
  );
});

// --- F4: requestWithRetry cold-start retry ---
test("requestWithRetry: returns populated second result when first is empty", async () => {
  // We test the behavior by constructing a fake server that counts calls.
  // requestWithRetry is not exported (intentionally private to the tool),
  // so we test the retry pattern directly via the exported helpers' contract.
  // Instead, we verify the pattern with a minimal mock:
  let callCount = 0;
  const fakeMgr = {
    async request(_method: string, _params: unknown, _file: string) {
      callCount++;
  ...
  // Import the module to get requestWithRetry — it's not exported,
  // so we re-implement the exact same logic for this test.
  async function retry(
    ...
    await new Promise((r) => setTimeout(r, 50)); // shortened for tests
  ...
  assert.equal(callCount, 2, "should have retried after empty first result");
  assert.deepEqual(result, [{ name: "found" }]);
});

test("requestWithRetry: propagates error without retry on throw", async () => {
  const fakeMgr = {
    async request() {
