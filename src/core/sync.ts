...
export function ingestSessionFile(
  ...
  try {
    ...
    db.prepare("DELETE FROM session_entries WHERE session_id = ?").run(
      sessionId,
    );
...
export function restoreSession(
  ...
  const entries = db
    .prepare(
      "SELECT json FROM session_entries WHERE session_id = ? ORDER BY seq",
    )
  ...
  const target = join(
    targetDir,
    `${
      String(meta["ts"] ?? "restored").replace(/[:.]/g, "-")
    }_${sessionId}.jsonl`,
  );
