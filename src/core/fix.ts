    ...
    for (const dir of readdirSync(getSessionsRoot(agentDir))) {
      ...
      if (!best) {
        findings.push({
          ...
          detail:
            `store '${dir}' has ${sessions.length} sessions but none carry a header cwd; register manually or blueberry projects merge`,
        });
        continue;
      }
      if (dryRun) {
        findings.push({
          ...
          detail: `would register project for orphan store '${dir}' (root ${
            best[0]
        ...
        if (project.slug === dir) {
          findings.push({
            kind: "orphan-store-registered",
            detail:
              `registered project '${project.slug}' for orphan store (root ${
                best[0]
              }, ${sessions.length} sessions)`,
          });
        } else {
          findings.push({
            kind: "orphan-store-registered",
            detail:
              `orphan store '${dir}' resolved to existing project '${project.slug}' (${sessions.length} sessions now visible there)`,
          });
        }
      }
    }
  ...
  for (const project of registry.projects) {
    ...
    if (project.sessionStore === "in-repo" && !existsSync(storeDir)) {
      findings.push({
        ...
        detail:
          `project '${project.slug}' configured for in-repo sessions but ${storeDir} does not exist (fine until first session)`,
      });
      continue;
    }
    ...
    for (const session of listSessions(storeDir)) {
      if (session.cwd !== project.canonicalPath) {
        if (dryRun) {
          findings.push({
            kind: "cwd-normalized",
            detail: `would rewrite session ${
              session.id.slice(0, 8)
            } cwd '${session.cwd}' -> '${project.canonicalPath}'`,
          });
        } else {
      ...
      if (
        ...
        if (dryRun) {
          findings.push({
            ...
            detail: `would clear dangling parentSession on session ${
              session.id.slice(0, 8)
            }`,
          });
        } else {
          ...
          findings.push({
            kind: "parent-session-cleared",
            detail: `cleared dangling parentSession on session ${
              session.id.slice(0, 8)
            }`,
          });
        }
  ...
  for (const project of registry.projects) {
    ...
    if (!alive) {
      findings.push({
        kind: "stale-project",
        detail:
          `project '${project.slug}' canonical and alias paths all missing; keep (sessions preserved) or blueberry projects forget ${project.slug}`,
      });
    }
  }
  ...
  for (const project of registry.projects) {
    ...
    if (first && first !== project.slug) {
      findings.push({
        ...
        detail:
          `projects '${first}' and '${project.slug}' share remote ${project.gitRemote}; blueberry projects merge ${project.slug} --into ${first}`,
      });
    } else {
