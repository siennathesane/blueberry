  ...
  if (nameMatches.length > 1) {
    const list = nameMatches
      .map(
        (s, i) =>
          `${i + 1}. ${s.id.slice(0, 8)} ${
            new Date(s.mtimeMs).toISOString().slice(0, 16)
          }`,
      )
      .join("\n");
    throw new Error(
...
export function renderMessages(
      ...
      case "assistant": {
        ...
        for (const b of contentBlocks(m.content)) {
          if (b["type"] === "toolCall") {
            out.push(
              `  · tool ${b["name"]} (${
                typeof b["id"] === "string" ? b["id"].slice(0, 8) : "?"
              })`,
            );
          }
        }
  ...
  const header = `[messages ${
    slice.length > 0 ? `${slice[0]!.n}..${slice[slice.length - 1]!.n}` : "none"
  } of ${numbered.length} total]`;
...
export function renderMessage(
  ...
  if (!item) {
    throw new Error(
      `no message #${messageNumber} (range 1..${numbered.length})`,
    );
  }
  const m = messageEntry(item.entry)!;
