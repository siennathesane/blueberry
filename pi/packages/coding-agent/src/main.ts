...
import { BlueberryDbStore } from "./core/session-store-db.ts";
import { setSessionStore } from "./core/session-store.ts";
...
export async function main(args: string[], options?: MainOptions) {
	resetTimings();
	// FORK(blueberry): DB-only session persistence. When BLUEBERRY_DB points at
	// a blueberry.db, every session write lands there — JSONL files are never
	// created (they become an export format via `bb restore`). Stock behavior
	// (JSONL under the agent dir) is the default when the env is unset.
	if (process.env.BLUEBERRY_DB) {
		setSessionStore(new BlueberryDbStore(process.env.BLUEBERRY_DB));
	}
