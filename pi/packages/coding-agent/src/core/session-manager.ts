...
export function getDefaultSessionDir(cwd: string, agentDir: string = getDefaultAgentDir()): string {
	...
	getSessionStore().ensureDir(sessionDir);
	return sessionDir;
}
...
export function loadEntriesFromFile(filePath: string): FileEntry[] {
	return getSessionStore().readAll(filePath) ?? [];
}
...
function readSessionHeaderForDiscovery(filePath: string): SessionHeader | null {
	return getSessionStore().readHeader(filePath);
}
...
export function findMostRecentSession(sessionDir: string, cwd?: string): string | null {
	...
	try {
		const files = getSessionStore()
			.listKeys(resolvedSessionDir)
			.map((path) => ({ path, header: getSessionStore().readHeader(path) }))
			.filter(
				(file): file is { path: string; header: SessionHeader } =>
					file.header !== null &&
					(!resolvedCwd || sessionCwdMatches(getSessionHeaderCwd(file.header), resolvedCwd)),
			)
			.map(({ path }) => ({ path, mtime: new Date(getSessionStore().mtimeMs(path)) }))
...
async function buildSessionInfo(filePath: string): Promise<SessionInfo | null> {
	try {
		const entries = getSessionStore().readAll(filePath);
		...
		const modified =
			typeof lastActivityTime === "number" && lastActivityTime > 0
				...
				: !Number.isNaN(headerTime)
					? new Date(headerTime)
					: new Date(getSessionStore().mtimeMs(filePath));
...
async function listSessionsFromDir(
	...
	try {
		const files = getSessionStore().listKeys(dir);
...
export class SessionManager {
	...
	private constructor(
		...
		if (persist && this.sessionDir) {
			getSessionStore().ensureDir(this.sessionDir);
		}
	...
	private _setSessionFile(sessionFile: string, preloadedFileEntries?: FileEntry[]): void {
		...
		if (getSessionStore().exists(this.sessionFile)) {
			...
			if (this.fileEntries.length === 0) {
				...
				if (getSessionStore().sizeBytes(explicitPath) > 0) {
	...
	newSession(options?: NewSessionOptions): string | undefined {
		...
		if (this.persist) {
			...
			this.sessionFile = getSessionStore().sessionKey(this.getSessionDir(), fileTimestamp, this.sessionId);
		}
	...
	private _rewriteFile(): void {
		...
		getSessionStore().rewriteAll(this.sessionFile, this.fileEntries);
	}
	...
	_persist(entry: SessionEntry): void {
		...
		if (!hasAssistant) {
			if (this.flushed) {
				getSessionStore().appendEntry(this.sessionFile, entry);
			} else {
		...
		if (!this.flushed) {
			// first flush: create the session with everything buffered so far
			// (fileEntries already includes `entry` — _appendEntry pushes first)
			getSessionStore().createNew(this.sessionFile, this.fileEntries);
			this.flushed = true;
		} else {
			getSessionStore().appendEntry(this.sessionFile, entry);
		}
	...
	static open(path: string, sessionDir?: string, cwdOverride?: string): SessionManager {
		...
		if (cwdOverride === undefined && existsSync(resolvedPath)) {
			try {
				header = getSessionStore().readHeader(resolvedPath);
			} catch (error) {
				...
				preloadedFileEntries = getSessionStore().readAll(resolvedPath) ?? undefined;
	...
	static forkFrom(
		...
		const timestamp = new Date().toISOString();
		const fileTimestamp = timestamp.replace(/[:.]/g, "-");
		const newSessionFile = getSessionStore().sessionKey(dir, fileTimestamp, newSessionId);
		...
		getSessionStore().createNew(newSessionFile, [
			newHeader,
			...sourceEntries.filter((e) => e.type !== "session"),
		]);
	...
	static async listAll(
		...
		try {
			...
			const allFiles = getSessionStore().listAllKeys(sessionsDir);
